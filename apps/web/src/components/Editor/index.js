import { bundlerUrl, cdnUrl } from "@/constains";
import React, {
  useState,
  useRef,
  useEffect,
...
const Editor = ({
  ...
  const firstNonHiddenFile = Object.entries(files).find(
    ([_, data]) => !data?.hidden
  )?.[0];
  const initialActiveFile =
    firstPassedActiveFileRef.current || firstNonHiddenFile;
  ...
  const resize = useCallback(
    (e) => {
      if (isResizing && resizeContainerRef.current) {
        const containerRect =
          resizeContainerRef.current.getBoundingClientRect();
  ...
  const getAllUsedPackages = useCallback((code) => {
    ...
    const packageNames = matches
      .map((match) => match[1] || match[2])
      .filter((pkg) => !pkg.startsWith("./") && !pkg.startsWith("@/"))
      .filter(
        (pkg) =>
          !(
            (template === "nextjs" || template === "nextjs-ts") &&
            pkg.startsWith("next/")
          )
      )
      .map((pkg) => {
    ...
    const filteredPackages = packageNames.filter(
    ...
    setUsedDeps((prev) => {
      ...
      packages.forEach(async (pkg) => {
        ...
        if (prev?.global?.[pkgName]) {
          ...
          filePackages[pkgName] =
            existingVersion || (await fetchLatestVersion(pkgName));
        }
      });
      newDeps.files = {
        ...prev?.files,
  ...
  const memoizedDependencies = useMemo(
    () => ({
      ...(usedDeps?.global || {}),
      ...(usedDeps?.files
        ? Object.values(usedDeps.files).reduce(
            (acc, fileDeps) => ({
              ...acc,
              ...fileDeps,
            }),
  ...
  const handleCodeUpdate = useCallback(
    (newCode) => {
      ...
      const prevPackages = getAllUsedPackages(
        filesState[activeFile]?.code || ""
      );
  ...
  const initFrameworkConfigFiles = async (framework) => {
    ...
    const configFiles = await getUIConfigFiles(
      template,
      previewSettings,
      usedUiFrameworks,
      framework,
    ...
    if (mainExtension !== currentExtension) {
      return {
        ...
        [mainFileMap[template]]: {
          code: filesState[previewFile || activeFile]?.code,
        },
      };
    }
  ...
  return (
    <div
      className="w-full flex flex-col rounded-lg justify-center overflow-hidden"
      style={{ height: "80vh" }}
    >
        ...
        <div className="space-x-2 hidden sm:flex">
          ...
          {screenOptions.map((option) => (
            <Chip
              ...
              onSelect={() =>
                setSelectedScreen(
                  selectedScreen === option.value ? "" : option.value
                )
              }
              showX={false}
              color="blue"
