...
const DiffHighlightingEditor = ({
  ...
  const [isCompletionActive, setIsCompletionActive] =
    useState(enableCompletion);
  ...
  const initializeMonaco = useCallback(
    async (monacoInstance) => {
      if (initializationCompleted.current || typeof window === "undefined")
        return;
  ...
  return (
    <SandpackStack style={{ height: "100%", margin: 0, background: "#0a0a0a" }}>
      <div style={{ flex: 1, paddingTop: 8, background: "#0a0a0a" }}>
        {!isEditorReady && (
          <div style={{ height: "100%", background: "#0a0a0a", color: "#fff" }}>
            Loading...
          </div>
        )}
        <DiffEditor
          ...
          loading={
            <div
              style={{ height: "100%", background: "#0a0a0a", color: "#fff" }}
            >
              Loading editor...
            </div>
          }
      ...
      <div className="absolute top-3 right-4 flex space-x-2">
        ...
        {!hasChanges && showSave && (
          <UtilityButton text="SAVE" shorcut="⌘S" onClick={onSave} />
        )}
        {!hasChanges && (
          <motion.button
