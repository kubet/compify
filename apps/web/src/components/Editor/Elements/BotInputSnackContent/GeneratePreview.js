...
import { generatePreview, remapFiles } from '@/lib/api'
...
function GeneratePreview({ files, apply, close, setSnackHeight, activeFile, userPlan, deleteFile, handleFileSwitch }) {
    ...
    const onRemapFiles = async () => {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                setGeneratedFiles(Object.keys(formattedFiles));
                setRemappedFiles(formattedFiles);
                apply(formattedFiles);
                // handleFileSwitch(Object.keys(formattedFiles)[0]);
    ...
    return (
        <>
            <div className="flex flex-col gap-1 py-1">
                <span className="text-xs pb-1">Choose files to generate preview</span>
                ...
                >
                    {isLoading ? (
                        <>
                            ...
                            <span className="hidden md:block">Generating preview...</span>
                        </>
                    ) : (
                        <>
                            <span className="hidden md:block">{remappedFiles ? 'Accept' : 'Generate'}</span>
                            {remappedFiles ? (
