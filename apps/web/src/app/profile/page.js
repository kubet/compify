...
import { User, Mail, Phone, MapPin, Calendar, Edit, Camera, LogOut, FolderOpen, Code, Activity, Bell, Settings, ChevronRight, User2, Zap, Box, Bot, Eye, LineChart, LogOutIcon, PowerOffIcon, EyeOff, Copy, Terminal, Key, LifeBuoy, FileText, Shield, Cookie } from 'lucide-react';
...
import { changePassword, getUserSubscriptionUsage, generateCliToken, getCliToken, revokeCliToken, updateUserInfo, deleteUser } from '@/lib/api';
...
const UserProfilePage = () => {
    ...
    const navItems = [
        { id: 'profile', icon: User, label: 'Profile', desc: 'Plan & usage details' },
        // { id: 'notifications', icon: Bell, label: 'Notifications' },
        { id: 'terms-support', icon: LifeBuoy, label: 'Terms & Support', desc: 'Help & legal info' },
        { id: 'settings', icon: Settings, label: 'Settings', desc: 'Security settings' },
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'profile':
                return <ProfileContent isEditing={isEditing} setIsEditing={setIsEditing} handleSave={handleSave} setActiveTab={setActiveTab} />;
            case 'notifications':
    ...
    return (
        <div className="flex items-start justify-start px-4 mx-auto w-full max-w-7xl" style={{ height: 'calc(100vh - 72px)' }}>
            ...
            <div className="relative h-full transition-all duration-300 ease-in-out flex items-start justify-start lg:w-64 w-14">
                <div className="py-8 flex flex-col items-start">
                    <div className="sm:ml-2 ml-1 flex items-center gap-4 mb-8">
                        ...
                        <h1 className="text-lg font-bold text-left hidden lg:block">{userName}</h1>
                    </div>
                    <div className='flex flex-col sm:gap-0 gap-1'>
                        {navItems.map((item) => (
                            ...
                            >
                                <div className={`border p-[9px] rounded-2xl border-white/15 ${activeTab === item.id ? 'bg-[#191919] border-purple-800/90 text-white/60' : 'bg-[#191919] text-gray-500'}`}>
                                    <item.icon className="w-6 h-6 flex-shrink-0" />
                                </div>
                                <div className='hidden sm:flex flex-col justify-start items-start'>
                                    <span className={`text-[16px] whitespace-nowrap hidden lg:inline ${activeTab === item.id ? 'text-purple-500/70' : 'text-white/80'}`}>{item.label}</span>
...
const ProfileContent = ({ isEditing, setIsEditing, handleSave, setActiveTab }) => {
    ...
    const componentCredits = {
        ...
        percentage: Math.min(100, Math.round(((subscriptionPlan.toCredits - usage.availableCredits || 0) / (subscriptionPlan.toCredits || 1)) * 100))
    };

    const aiCredits = {
        total: subscriptionPlan.toAiCredits || 0,
        used: subscriptionPlan.toAiCredits - usage.availableAiCredits || 0,
        percentage: Math.min(100, Math.round(((subscriptionPlan.toAiCredits - usage.availableAiCredits || 0) / (subscriptionPlan.toAiCredits || 1)) * 100))
    };

    return (
        <div className="mx-auto w-full">
            <h2 className="text-4xl font-bold mb-8 bg-gradient-to-r from-gray-300 via-gray-500 to-gray-700 text-transparent bg-clip-text">My Profile</h2>
                ...
                <div className="w-full flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <UsageCard
                            ...
                            description={subscriptionPlan?.planPrice > 0 ? `${componentCredits.used} / ${componentCredits.total} used` : `${usage.availableCredits} available`}
                        ...
                        <UsageCard
                            title="AI Credits"
                            description={subscriptionPlan?.planPrice > 0 ? `${aiCredits.used} / ${aiCredits.total} used` : `${usage.availableAiCredits} available`}
...
const SubscriptionCard = ({ title, description, icon, colors, status, price, setActiveTab, endDate }) => (
    <motion.div
    ...
    >
        ...
        />
        <GradientSpot color={colors[0]} size={100} position={{ x: '0%', y: '-10%' }} opacity={0.2} />
        <GradientSpot color={colors[1]} size={180} position={{ x: '0%', y: '10%' }} opacity={0.2} />
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-red-400/80 text-sm font-medium">
                                Active until {endDate ? new Date(endDate).toLocaleDateString() : 'end of billing period'}
                            </p>
                        </div>
                    </motion.div>
                )}
...
const UsageCard = ({ title, description, icon, colors, percentage, showProgress }) => (
    <motion.div
        ...
        <div className="relative z-10">
            ...
            {showProgress && (
                <>
                    ...
                    <p className="text-right text-sm text-gray-400 mt-1">{percentage}% used</p>
                </>
            )}
        </div>
    </motion.div>
...
const NotificationsContent = () => (
    <div>
        <h2 className="text-4xl font-bold mb-8 bg-gradient-to-r from-gray-300 via-gray-500 to-gray-700 text-transparent bg-clip-text">Notifications</h2>
        {/* Notifications list */}
    </div>
);
...
const SettingsContent = () => {
    ...
    const handleGenerateToken = async () => {
        ...
        if (resp.status === 201 && resp.data.token) {
            ...
            setIsToastVisible(true);
            setToastMessage({ type: 'error', message: resp.data.message || 'Failed to generate CLI token!' });
        }
    ...
    const handleRevokeToken = async () => {
        ...
        if (resp.status === 201) {
            ...
            setIsToastVisible(true);
            setToastMessage({ type: 'error', message: resp.data.message || 'Failed to revoke CLI token!' });
        }
        setIsLoadingToken(false);
    };
    ...
    const handleChangePassword = async (e) => {
        ...
        if (resp.status === 201) {
            ...
            setIsToastVisible(true);
            setToastMessage({ type: 'error', message: resp.data.message || 'Failed to update password!' });
        }
    };
    ...
    return (
        <div className="mx-auto w-full overflow-hidden">
            <h2 className="text-4xl font-bold mb-8 bg-gradient-to-r from-gray-300 via-gray-500 to-gray-700 text-transparent bg-clip-text">Account Settings</h2>
            <div className="flex flex-col gap-8 mb-8 w-full">
                <div className="w-full flex flex-col gap-6">
                    <motion.div
                        className="relative cursor-pointer rounded-3xl shadow-md overflow-hidden border border-[rgba(255,255,255,0.1)] p-6 w-full h-full flex flex-col"
                    ...
                    >
                        ...
                        />
                        <GradientSpot color="#10b981" size={100} position={{ x: '0%', y: '-10%' }} opacity={0.2} />
                        <GradientSpot color="#059669" size={180} position={{ x: '0%', y: '10%' }} opacity={0.2} />

                        <div className="flex items-center gap-3 mb-4">
                            <Terminal className="w-6 h-6 text-emerald-500" />
                            <h3 className="text-2xl font-semibold">CLI Access Token</h3>
                        </div>
                        <p className="text-gray-400 mb-6">Securely manage your CLI access token for command-line interactions with the platform.</p>
                    ...
                    >
                        ...
                        />
                        <GradientSpot color="#9333ea" size={100} position={{ x: '0%', y: '-10%' }} opacity={0.2} />
                        <GradientSpot color="#6b21a8" size={180} position={{ x: '0%', y: '10%' }} opacity={0.2} />

                        <div className="flex items-center gap-3 mb-4">
                            <User2 className="w-6 h-6 text-purple-500" />
                            <h3 className="text-2xl font-semibold">Personal Information</h3>
                        </div>
                        <p className="text-gray-400 mb-6">Update your name and personal details.</p>
                    ...
                    >
                        ...
                        />
                        <GradientSpot color="#3b82f6" size={100} position={{ x: '0%', y: '-10%' }} opacity={0.2} />
                        <GradientSpot color="#1e40af" size={180} position={{ x: '0%', y: '10%' }} opacity={0.2} />
                        <h3 className="text-2xl font-semibold mb-4">Account Security</h3>
                        <p className="text-gray-400 mb-4">Update your password easily to maintain your account&apos;s security and privacy.</p>
                    ...
                    <p className="text-gray-400 w-full mt-8 text-right">
                        Need help? <a href="#" onClick={handleContactClick} className="text-purple-500 hover:text-purple-400 transition-colors">Contact us</a>
                    </p>
                    <motion.div
                    ...
                    >
                        ...
                        <p className="text-gray-400 mb-4">Deleting your account is permanent and cannot be undone. Please proceed with caution.</p>
                        <Button
                            text="Delete Account"
                            variant="full"
                            color="red"
...
const TermsSupportContent = () => {
    ...
    return (
        <div className="mx-auto w-full overflow-hidden">
            <h2 className="text-4xl font-bold mb-8 bg-gradient-to-r from-gray-300 via-gray-500 to-gray-700 text-transparent bg-clip-text">Terms & Support</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-2">
                    {links.map((item, index) => (
                        ...
                        >
                            ...
                            />
                            <GradientSpot color={item.color} size={100} position={{ x: '0%', y: '-10%' }} opacity={0.2} />
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-gray-200">
                                <span className="hidden sm:inline">Need help? Our support team is here to assist you (support@compify.app)</span>
                                <span className="sm:hidden">Contact support team (support@compify.app)</span>
                            </p>
                        </div>
                        <Button
                            text="Contact"
