...
export class ConstructImageService {
  ...
  async constructOGImage(
    ...
    const segmentText = (text: string, maxCharsPerLine = 14, maxLines = 4): string => {
    ...
    console.log('og image created', result);
    return {
      buffer: finalImage,
      mimetype: 'image/webp', // Changed mimetype to webp
    };
  ...
  async constructAnimatedImg(captures: string[]): Promise<string> {
    try {
      ...
      const firstImage = await sharp(
        Buffer.from(captures[0].replace(/^data:image\/\w+;base64,/, ''), 'base64')
      );
      ...
      const scale = 0.6;
      const standardWidth = Math.min(Math.max(Math.floor(firstImageMetadata.width * scale), 1), 16777215);
      const standardHeight = Math.min(Math.max(Math.floor(firstImageMetadata.height * scale), 1), 16777215);
      ...
      console.log(`Successfully created animation with ${frames.length} frames`);
