...
export class ComponentService {
  ...
  async share(id: string, user: User) {
    ...
    if (
      ...
      await this.constructImageService.constructOGImage(id, component.name);
    }
  ...
  async fork(body: { componentId: string }, user: User) {
    ...
    if (
      originalComponent.visibility === ComponentVisibility.PRIVATE ||
      originalComponent.visibility === ComponentVisibility.DRAFT
    ) {
  ...
  async checkIfPublishingDomainIsAvailable(
    publishingDomain: string,
    id: string,
  ) {
    const q = await this.componentRepository
      .createQueryBuilder('component')
      .where('component.publishingDomain = :domain', {
  ...
  async create(createComponentDto: CreateComponentDto, user: User) {
    ...
    if (createComponentDto?.publishingName) {
      ...
      await this.checkIfPublishingDomainIsAvailable(
        publishingDomain,
  ...
  async updateComponentImageUploaded(id: string) {
    if (id) {
      await this.componentRepository.update(shortIdToUuid(id), {
        imageUploaded: true,
      });
    }
  }
  ...
  async viewOne(id: string) {
    ...
    if (id.startsWith('@')) {
      ...
      if (!component) {
        throw new NotFoundException(
          `Component with publishing domain "${id}" not found`,
        );
      }
