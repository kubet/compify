...
export class UserService {
  ...
  async signUpContinue(user: User, signUpDto: SignUpDto) {
    const userFound = await this.userRepo.findOneBy({ email: user.email });
    if (!user) {
  ...
  async verifyEmail(b: VerifyAccountDto) {
    ...
    try {
      ...
      const whoAmIData = await this.whoami(userRepo);
      const plan = await this.getUserSubscriptionPlan(userRepo);
      const accessToken = await this.getAccessToken({
        id: userRepo.id,
        plan,
      });
  ...
  async validateUserPassword(authCredentialsDto: AuthCredentialsDto) {
    ...
    if (!user?.valid) {
      throw new NotFoundException('Please verify your email first.');
    }
  ...
  async signIn(
    ...
    if (!resp) {
      throw new InternalServerErrorException('Wrong credentials try again!');
    }
