...
export class CliService {
  ...
  async publishStory(body: unknown, authorization?: string) {
    ...
    const storedFiles = Object.fromEntries(
      Object.entries(normalized.files).map(([filePath, code]) => [
        filePath,
        { code },
      ]),
    );
    const component = await this.componentService.create(
      {
        ...
        usedDeps: { global: normalized.dependencies, files: {} },
        pageSettings: {
          storybook: {
    ...
    const publishingDomain = component.publishingDomain;
    ...
    const publiclyAddressable = normalized.visibility !== 'private';
    return {
  ...
  private validatePublishStory(value: unknown): PublishStoryDto {
...
export const MAX_STORY_FILES = 100;
