...
export type SandpackStaticMessage = BaseSandpackMessage &
  (
    | {
        type: "compile";
    ...
    | { type: "urlchange"; url: string; back: boolean; forward: boolean }
    | { type: "refresh" }
  );

/**
 * Compatibility messages used by Sandpack React hooks. Server templates are
 * rejected by this client and no server emulator implementation is included.
 */
export type SandpackShellMessage = BaseSandpackMessage &
  (
    | { type: "connected" }
    | { type: "shell/restart" }
    | { type: "shell/openPreview" }
    | { type: "stdout"; payload: { data?: string; type?: "out" | "err" } }
...
export type SandpackMessage =
  ...
  | SandpackStaticMessage
  | SandpackShellMessage;
