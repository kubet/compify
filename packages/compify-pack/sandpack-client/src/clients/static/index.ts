...
import { fromBundlerFilesToFS, generateRandomId } from "./client-utils";
...
export class SandpackStatic extends SandpackClient {
  ...
  private async compile(files: Record<string, Uint8Array>): Promise<void> {
  ...
  public dispatch(message: SandpackMessage): void {
    switch (message.type) {
      case "compile":
        this.compile(message.modules as Record<string, Uint8Array>);
        break;
