...
type NotionProperties = {
  ...
  Slug?: { formula: { string: string } } | { rich_text: Array<{ plain_text: string }> };
  Excerpt?: { rich_text: Array<{ plain_text: string }> };
  Category?: { select: { name: string } };
  Published?: { checkbox: boolean; date?: { start: string } };
  Date?: { date: { start: string } };
  Image?: { files: Array<{ type: string; file?: { url: string }; external?: { url: string } }> };
};
...
async function notionPageToBlogPost(page: NotionPage): Promise<BlogPost> {
  try {
    ...
    const content = blocksToHtml(blocksResponse.results as BlockObjectResponse[]);
    ...
    if (page.cover) {
      ...
      imageSrc = imageFile.type === 'external' ? imageFile.external?.url || imageSrc : imageFile.file?.url || imageSrc;
    }
...
export async function getNotionPostBySlug(slug: string): Promise<BlogPost | null> {
  ...
  try {
    ...
    console.error(`Failed to fetch post with slug "${slug}" from Notion:`, error);
    return null;
  }
}
